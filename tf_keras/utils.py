...
def train_data_generator(train_directory, batch_size, preprocessing_function=None, target_size=(224, 224)):
...
def test_data_generator(test_directory, preprocessing_function=None, target_size=(224, 224)):
...
def show_result_pic(image, preds):
    ...
    draw.text((0, 0), '%s: %f' % (preds[0], preds[1]), (255, 0, 0), font=font)
    image.show()
...
def decode_prediction(preds, class_list):
    preds = preds[0]
    c = np.argmax(preds)
    result = [class_list[c], preds[c]]
    return result
...
def data_split(data_path, new_main_path, test_ratio=0.1, val_ratio=0.1, seed=15):
    ...
    for classname in os.listdir(data_path):
        ...
        cp_filename(data_path, os.path.join(new_main_path, 'validation'), classname, val_list)
